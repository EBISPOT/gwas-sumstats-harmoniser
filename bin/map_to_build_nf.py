...
def merge_ss_vcf(ss, vcf, from_build, to_build, chroms, coordinate):
    ...
    if build_map:
        ssdf[BP_DSET] = [lft.map_bp_to_build_via_liftover(chromosome=x, bp=str(int(y)-int(coordinate[0])), build_map=build_map) for x, y in zip(ssdf[CHR_DSET], ssdf[BP_DSET])]
    for chrom in chroms:
        print(chrom)
