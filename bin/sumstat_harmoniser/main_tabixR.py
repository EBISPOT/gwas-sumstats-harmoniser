...
def main():
    ...
    #--------------------------------- YUE-------------------------------------------------------
    sum=0
    # Prepare query of tabix file to return
    ssdf = pd.read_csv(args.sumstats, sep=args.in_sep, dtype=str)
    pos = ssdf.loc[:,[args.chrom_col,args.pos_col,args.pos_col]]
    pos.columns = [args.chrom_col,"start","end"]
    
    # sort 1000,000 sites used 2.5s in python V.S. 9.6s in shell -> better sort in python
    pos[["start","end"]] = pos[["start","end"]].apply(pd.to_numeric, errors='ignore')
    
    # inout file of tabix -R File nees to be sorted, no header and no NA
    sites=(pos.dropna()).sort_values(by=[args.chrom_col,"start"])
    sites.to_csv(args.hm_statfile+"query", sep="\t",header=False,index=False)
    
    # run tabix in shell and transfer the stdout (bytes) into a dataframe to merge
    tabix_running=subprocess.run(['tabix', '-R', args.hm_statfile+"query", args.vcf],stdout=PIPE)
    reference = pd.read_csv(StringIO(tabix_running.stdout.decode("utf-8")), sep=r"\t", dtype=str)
    reference.columns=['chrom','pos','ref_rsID','ref_other_allele','ref_effect_allele','ref_QUAL','ref_FILTER','ref_INFO']

    # Creat db for query
    conn = sql.connect("file::memory:")
    # Delet table if the name already existed
    cur = conn.cursor()
    cur.execute("DROP TABLE if EXISTS reference")
    # write dataframe into a database
    reference.to_sql('reference', conn)
    # index with chrom and pos
    # cur.execute("CREATE INDEX new_idx ON reference (pos, chrom)")
    cur.execute("CREATE INDEX new_idx ON reference (pos, chrom, ref_rsID, ref_other_allele, ref_effect_allele, ref_QUAL, ref_FILTER, ref_INFO)")
    #--------------------------------- YUE-------------------------------------------------------
    ...
    for counter, ss_rec in enumerate(yield_sum_stat_records(args.sumstats,
        ...
        if not ss_rec.hm_code:
            ...
            start_vcf=timeit.default_timer()
            vcf_recs = get_vcf_records(reference,ss_rec.chrom,ss_rec.pos,conn)
            end_vcf=timeit.default_timer()
            sum=sum+(end_vcf-start_vcf)
    ...
    print("Done!","get_vcf",sum,"total_time:",time)
...
def get_vcf_records(df,chrom,pos,conn):
    ...
    #chr=str(chrom)
    #site=str(pos)
    #query = "SELECT * FROM reference WHERE chrom==? AND pos ==?"
    query="SELECT pos, chrom, ref_rsID, ref_other_allele, ref_effect_allele, ref_QUAL, ref_FILTER, ref_INFO FROM reference WHERE chrom==? AND pos ==?"
    response = pd.read_sql(query, conn,params=(chrom,pos)).dropna(axis=1).values.tolist()
    #response =df.query('chrom==@chr & pos==@site').dropna(axis=1).values.tolist()
